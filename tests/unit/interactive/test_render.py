...
def test_settings_highlights_only_the_active_choice() -> None:
    ...
    text = stream.getvalue()
    # Active choices are bold yellow; the rest are dim.
    assert "\x1b[1;33mfalse\x1b[0m" in text
    assert "\x1b[2mtrue\x1b[0m" in text
    assert "\x1b[1;33mengineering\x1b[0m" in text
...
def test_settings_dims_an_unfocused_disabled_row() -> None:
    ...
    assert "\x1b[1;33mtrue\x1b[0m" not in text
...
def test_a_focused_disabled_row_keeps_the_cursor_highlight() -> None:
    ...
    text = stream.getvalue()
    assert "\x1b[1;33mtrue\x1b[0m" in text
...
def test_hybrid_rows_render_single_value_not_inline_choices() -> None:
    ...
    render_settings(console, rows=rows, selected=0, file_path="/tmp/config.env")
    text = stream.getvalue()
    assert "\x1b[1;33m30\x1b[0m" in text
